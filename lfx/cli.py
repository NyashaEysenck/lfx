...
# v5 over the earlier 3B: the same 0.765 form accuracy on the 485 human-labelled
# passages, but measured honestly (three of that set's straw man records had leaked
# into training) and better on every other field — argument_type 0.842 vs 0.816,
# premise F1 0.743 vs 0.718. It also speaks schema v2.0, so its `form` is a list.
#
# 3B over the 1.5B v4: 0.765 vs 0.699 form accuracy, largely untangling straw man
# from ad hominem, the pair v4 confused most. Costs roughly double the size and
# per-query time.
#
...
DEFAULT_MODEL = "models/mlx_v5_8bit"
